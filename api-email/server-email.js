...
const transporter = nodemailer.createTransport({
  ...
  secure: false,
  auth: {
    user: 'ba.edison@gmail.com',
    pass: 'SENHA_APP_16_CARACTERES_AQUI'  // ⚠️ Gere em: https://myaccount.google.com/apppasswords
  }
});
...
app.post('/server-email', async (req, res) => {
  try {
    ...
    // Configurar o email
    const mailOptions = {
      from: '"Agenda Cultural" <ba.edison@gmail.com>',
...
app.get('/health', (req, res) => {
  res.json({ status: 'OK', service: 'api-email', timestamp: new Date() });
});
...
// Iniciar servidor
app.listen(PORT, () => {
  console.log(`🚀 API de Email rodando na porta ${PORT}`);
